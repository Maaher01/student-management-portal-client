...
app.put('/update/:id', (request, response) => {
    const id = parseInt(request.params.id)
    const { name, department, semester, cgpa, mobile, email, dob, gender, address, fatherName, motherName } = request.body
    console.log("request", request.body)
    pool.query('UPDATE students SET full_name = $1, department = $2, semester_no = $3, current_cgpa = $4, mobile_no = $5, email = $6, dob = $7, gender = $8, address = $9, father_name = $10, mother_name = $11 WHERE id = $12', [name, department, semester, cgpa, mobile, email, dob, gender, address, fatherName, motherName, id], (error, results) => {
        if (error) {
            throw error
        }
